...
(function (root) {
  ...
  var VV = root.MRPVideo, HE = root.MRPHeic;
  var T = I.T;
  var items = [];
  var busy = false;
  var PRESET = 'normal';
  ...
  var SUPPORTED = ['png', 'jpg', 'jpeg', 'webp', 'svg', 'gif', 'bmp', 'mp4', 'mov', 'm4v', 'mkv', 'webm', 'avi', 'heic', 'heif'];
  var STRIP_OK = ['png', 'jpg', 'jpeg', 'webp', 'svg'];
  function isVideoExt(ext) { return VV && VV.isVideoExt(ext); }
  ...
  function metaValue(m) {
    var keys = ['exif_present', 'last_modified', 'gps_present', 'binary_data', 'mdta', 'mp4_custom_present'];
  ...
  async function scanBytes(bytes, ext) {
    var scan;
    if (isVideoExt(ext)) scan = VV.scanVideo(bytes, ext);
    else if (isHeicExt(ext) && HE) scan = HE.scanHeic(bytes);
    else if (STRIP_OK.indexOf(ext) >= 0) scan = E.scanImage(bytes, ext);
    else scan = { verdict: 'clean', findings: [], metadata: [], c2pa: false, width: 0, height: 0 };
    if (wmMode() && !isVideoExt(ext)) {
      try {
        var rgba = null, w = 0, h = 0;
        if (isHeicExt(ext) && HE) {
          var dec = await HE.decode(bytes);
          rgba = dec.rgba; w = dec.width; h = dec.height;
        } else {
          var bmp = await createImageBitmap(new Blob([bytes]));
          w = bmp.width; h = bmp.height;
          var c0 = document.createElement('canvas');
          c0.width = w; c0.height = h;
          var ctx0 = c0.getContext('2d', { willReadFrequently: true });
          ctx0.drawImage(bmp, 0, 0);
          bmp.close();
          rgba = ctx0.getImageData(0, 0, w, h).data;
        }
        if (rgba) {
          var r = A.detectInvisible(rgba, w, h);
  ...
  async function deepClean(bytes, ext, opts, forgeIso) {
    var bmp = await createImageBitmap(new Blob([bytes]));
    var c = document.createElement('canvas');
    ...
    bmp.close();
    return processCanvas(c, ext, opts, forgeIso, null);
  }
  async function processCanvas(c, ext, opts, forgeIso, label) {
    var ctx = c.getContext('2d', { willReadFrequently: true });
    var removed = [];
    ...
    if (opts.forge_pixel) {
      root.MRPForge.applyPixel(c, forgeIso || 200);
      removed.push(T('forgepixel'));
    }
    removed.push(label || T('removed_deep', { l: opts.anti_wm || 'light' }));
    var blob, extOut = ext;
  ...
  async function cleanOne(item, statusEl, btn, actions) {
    ...
    try {
      ...
      var F = root.MRPForge;
      var forgeMeta = null;
      if (opts.forge_meta || opts.forge_icc) {
        var sess = new F.ForgeSession({ gps: opts.forge_gps, region: opts.forge_region });
        forgeMeta = sess.forFile(bytes, item.ext);
      }
      var pixelPath = (PRESET === 'deep') || opts.recompress || opts.remove_visible || opts.anti_wm || opts.forge_pixel;
      var blob, extOut, removed = [], note = '';
      if (isVideoExt(item.ext)) {
        // 视频: 容器级剥离, 不转码(深度模式亦然, 逐帧处理见桌面版/云适配器)
        var sv = VV.stripVideo(bytes, item.ext);
        if (!sv || !sv.data) throw new Error('unsupported');
        blob = new Blob([sv.data], { type: item.file.type || 'application/octet-stream' });
        extOut = item.ext;
        removed = sv.removed.length ? sv.removed : [T('video_container_clean')];
        note = (PRESET === 'deep' || opts.anti_wm) ? T('video_deep_note') : '';
      } else if (isHeicExt(item.ext)) {
        var dec = await HE.decode(bytes).catch(function (err) {
          throw new Error(T('heic_failed') + ': ' + (err && err.message || '?'));
        });
        var hc = canvasFromRgba(dec.rgba, dec.width, dec.height);
        var dh = await processCanvas(hc, 'jpg', opts, forgeMeta ? forgeMeta.iso : 0, T('removed_heic'));
        blob = dh.blob; extOut = 'jpg'; removed = dh.removed; note = dh.note;
      } else if (item.ext === 'svg' && pixelPath) {
        ...
        var d = await deepClean(bytes, item.ext, opts, forgeMeta ? forgeMeta.iso : 0);
        ...
        var d2 = await deepClean(bytes, item.ext, { remove_visible: false, recompress: true, anti_wm: '' }, 0);
        blob = d2.blob; extOut = d2.extOut; removed = d2.removed;
      }
      // 伪造元数据注入(字节级, 像素无损; 视频跳过)
      if (forgeMeta && extOut !== 'svg' && !isVideoExt(item.ext)) {
        var outB = new Uint8Array(await blob.arrayBuffer());
        var injected = null;
        if (extOut === 'jpg' || extOut === 'jpeg') injected = F.injectJpegMeta(outB, forgeMeta.exifTiff, forgeMeta.xmpPacket, opts.forge_icc ? forgeMeta.icc : null);
        else if (extOut === 'png') injected = await F.injectPngMeta(outB, forgeMeta.exifTiff, forgeMeta.xmpPacket, opts.forge_icc ? forgeMeta.icc : null);
        else if (extOut === 'webp') injected = F.injectWebpMeta(outB, forgeMeta.exifTiff, forgeMeta.xmpPacket, opts.forge_icc ? forgeMeta.icc : null);
        if (injected) blob = new Blob([injected], { type: blob.type || 'application/octet-stream' });
        note = (note ? note + ' | ' : '') + T('forged_prefix') + forgeMeta.summary;
      } else if (forgeMeta && extOut === 'svg') {
        note = (note ? note + ' | ' : '') + T('svg_no_camera');
      } else if (forgeMeta && isVideoExt(item.ext)) {
        note = (note ? note + ' | ' : '') + T('video_no_forge');
      }
  ...
  function renderRow(item, container) {
    ...
    if (item.isVideo) {
      ...
      vd.src = purl; vd.preload = 'metadata'; vd.muted = true; vd.playsInline = true;
      t.appendChild(vd);
    } else if (item.isImage) {
      var im = el('img');
      im.src = purl; im.loading = 'lazy'; im.draggable = false;
      t.appendChild(im);
    } else t.appendChild(el('div', 'file-ico', item.isHeic ? '🖼️' : '📄'));
    row.appendChild(t);
    ...
    if (item.scan && item.scan.width && item.scan.height) {
      meta.appendChild(el('span', 'badge', item.scan.width + '×' + item.scan.height +
        (item.scan.duration ? ' · ' + item.scan.duration + 's' : '')));
    }
        ...
        jobs.push(file.arrayBuffer().then(function (buf) {
          ...
          return scanBytes(bytes, ext).then(function (scan) {
            return { id: 'f' + (++uid), file: file, name: file.name, size: file.size, ext: ext,
              scan: scan,
              isVideo: isVideoExt(ext), isHeic: isHeicExt(ext),
              isImage: !isVideoExt(ext) && !isHeicExt(ext),
  ...
  function collectOptions() {
    return {
      ...
      forge_meta: getEl('opt_forgemeta').checked,
      forge_gps: getEl('opt_forgegps').checked ? 'random' : 'off',
      forge_region: getEl('opt_forgeregion').value,
      forge_icc: getEl('opt_forgeicc').checked,
      forge_pixel: getEl('opt_forgepixel').checked,
    };
  }
  ...
  function updateForgeLinks() {
    var on = getEl('opt_forgemeta').checked;
    getEl('sw_forgegps').classList.toggle('disabled', !on);
    getEl('opt_forgeregion').disabled = !on;
    if (!on) getEl('opt_forgegps').checked = false;
  }
  ...
  function init() {
    ...
    ['opt_visible', 'opt_recompress', 'opt_antiwm', 'opt_forgemeta', 'opt_forgegps', 'opt_forgeregion', 'opt_forgeicc', 'opt_forgepixel'].forEach(function (id) {
      getEl(id).addEventListener('change', markCustom);
    });
    getEl('opt_forgemeta').addEventListener('change', updateForgeLinks);
    pickPreset('normal');
    updateForgeLinks();
    applyLang();
    applyTheme();
